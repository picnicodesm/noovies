...
export const moviesApi: Fetchers<MovieResponse> = {
  ...
  search: ({ queryKey }) => {
    const [_, query] = queryKey; // es6
    return fetch(
      `${BASE_URL}/search/movie?api_key=${API_KEY}&language=en-US&page=1&query=${query}&region=KR`
    ).then((res) => res.json());
  },
  detail: ({ queryKey }) => {
    const [_, id] = queryKey; // es6
    return fetch(
...
export const tvApi: Fetchers<TVResponse> = {
  ...
  search: ({ queryKey }) => {
    const [_, query] = queryKey; // es6
    return fetch(
      `${BASE_URL}/search/tv?api_key=${API_KEY}&language=en-US&page=1&query=${query}&region=KR`
    ).then((res) => res.json());
  },
  detail: ({ queryKey }) => {
    const [_, id] = queryKey; // es6
